<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">

<HTML>
<HEAD>
<TITLE>ReallyRareWares</TITLE>

</HEAD>
<BODY bgcolor="#FFFFFF">

<table border="0" cellpadding="2" cellspacing="0" width="100%">
  <!-- Titulo -->
  <tr>
  <td>
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
    <tr><td align="center"><img src="img/title.gif"></td>
    </tr>
  </table>
  </td>
  </tr>

  <tr>
  <td bgcolor="#6699CC"><img height=15 width=1 src="dot.gif" alt="null"></td>
  </tr>

  <tr>
  <td>
  <table border="0" cellpadding="0" cellspacing="0" width="100%">
    <tr valign="top">
    <!-- Menu -->
    <td width=120 nowrap>
    <table border="0" cellpadding="0" cellspacing="0">
      <tr valign="top">
      <td bgcolor="#999999" width=119 align="center" nowrap>
      <font size="1" face="Tahoma">
      <table border="0" cellpadding="0" cellspacing="0" width="120">
    
    </td>
      </table>
      <table border="0" cellpadding="0" cellspacing="0" width="120">
    <tr><td bgcolor="#669999"><font face="Tahoma" size="2">
      <body link="#999999" vlink="#999999" alink="#999999">
        <center><img src="img/menu.gif"></center>
    </font></td></tr>
      </table>
<br>
    <?php include("menu.htm") ?>


      <table border="0" cellpadding="0" cellspacing="0" width="120">
    <tr><td></td></tr>
    <tr></tr>
    <tr><td></td></tr>
    <tr><td align="center"><font face="verdana" size="1">
       </font></td></tr>
    <tr><td align="center">
      <table border="0" cellpadding="0" cellspacing="0">
        <tr>

        </tr>
      </table>
    </td></tr>
      </table>
<br><br><br><br><br><br><br><br><br><br><br><br><br><br><br>
      <table border="0" cellpadding="0" cellspacing="0" width="120">
    <tr><td height=250><img width=1 height=100 src="dot.gif"></td></tr>
      </table>
      </font>
      </td>
     
      </tr>
    </table></td>
    <td width="100%">
    <table border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr><td>
        <table border="0" cellpadding="0" cellspacing="0" width="100%">
      <tr>
      <td width="100%" align="right">
      </td>
      </tr>
    </table></td></tr>
      <tr><td>
        <table border="0" cellpadding="10" cellspacing="0" width="100%">
      <tr><td width="450"></td>
        <td width="20%"></td></tr>
      <tr><td>
        <font face="Geneva,Verdana,Helvetica,Arial">

   
        <table border="0" cellpadding="2" cellspacing="0" width="20">
          <tr bgcolor="669999"><td width="20"><img width=20 height=1 src="dot.gif"></td>
          <td><font face="Tahoma" size="4" color="black"><img src=timeline2.gif></font></td>
          <td width="20"><img width=20 height=1 src="dot.gif"></td></tr>
        </table>
        </a>
        <body link="#000000" vlink="#000000" alink="#000000">
        <font face="Verdana" size=2 color="6699CC">
          <br><div align="justify"> <b>  </b></div> <p>
        </font>
        <font face="Tahoma" size=2 color="black">
        
          <!-- Begin Timeline -->

             <!-- Begin Timeline Module 1998 -->

               <table border="0" cellpadding="0" cellspacing="0" width="100%">
                 <tr valign="top">

                 <td width=80 nowrap>
                    <table border="0" cellpadding="0" cellspacing="0">
                    <tr valign="top">
                    <td bgcolor="#FF9999" width=79 align="center" nowrap>
                    <font size="1" face="Tahoma">
                    <table border="0" cellpadding="0" cellspacing="0" width="80">
    
                 </td>
                    </table>

               <br>
                     <table border="0" cellpadding="0" cellspacing="0" width="80">
                   <tr>
                   <td><font face="Tahoma" size="2">
                    <center><img src=1998.gif> </center></font></td></tr>
                       <td><font face="verdana" size="1">
               
                      </body>
               
                     <table border="0" cellpadding="0" cellspacing="0" width="80">
                   <tr><td></td></tr>
                   <tr></tr>
                   <tr><td></td></tr>
                   <tr><td align="center"><font face="verdana" size="1">
                     </font></td></tr>
                   <tr><td align="center">
                     <table border="0" cellpadding="0" cellspacing="0">
                       <tr>
                       
                       </tr>
                     </table>
                   </td></tr>
                     </table>

                     <table border="0" cellpadding="0" cellspacing="0" width="80">
                   <tr><td height=30><img width=1 height=10 src="dot.gif"></td></tr>
                     </table>
                     </font>
                     </td>
                    
                     </tr>
                   </table></td>
                                      <td width="100%">
                   <table border="0" cellpadding="0" cellspacing="0" width="100%">
                     <tr><td>
                       <table border="0" cellpadding="0" cellspacing="0" width="100%">
                     <tr>
                     <td width="100%" align="right">
                     </td>
                     </tr>
                   </table></td></tr>
                     <tr><td>
                       <table border="0" cellpadding="10" cellspacing="0" width="100%">
                     <tr><td width="650"></td>
                       <td width="0%"></td></tr>
                     <tr><td>
                       <font face="Geneva,Verdana,Helvetica,Arial">                       
                      <!-- Body -->
                <ul>
                    <li>Something interesting happens</li>

                    <li>Another interesting thing happens</li>

                    <li>Something not so interesting happens</li>

                    <li>blahblah</li>

                </ul>
               
                       </blockquote></font>
               
                       <br><br>
               
                       </font>
                     </td></tr>
                   </table>
                     </td></tr>
                   </table>
                   </td>
                   </tr>
                 </table></td>
                 </tr>
               </table> 
             <!-- End Timeline Module 1998 -->
              <br>
             <!-- Begin Timeline Module 1999 -->

               <table border="0" cellpadding="0" cellspacing="0" width="100%">
                 <tr valign="top">

                 <td width=80 nowrap>
                    <table border="0" cellpadding="0" cellspacing="0">
                    <tr valign="top">
                    <td bgcolor="#FF9999" width=79 align="center" nowrap>
                    <font size="1" face="Tahoma">
                    <table border="0" cellpadding="0" cellspacing="0" width="80">
    
                 </td>
                    </table>

               <br>
                     <table border="0" cellpadding="0" cellspacing="0" width="80">
                   <tr>
                   <td><font face="Tahoma" size="2">
                    <center><img src=1999.gif> </center></font></td></tr>
                       <td><font face="verdana" size="1">
               
                      </body>
               
                     <table border="0" cellpadding="0" cellspacing="0" width="80">
                   <tr><td></td></tr>
                   <tr></tr>
                   <tr><td></td></tr>
                   <tr><td align="center"><font face="verdana" size="1">
                     </font></td></tr>
                   <tr><td align="center">
                     <table border="0" cellpadding="0" cellspacing="0">
                       <tr>
                       
                       </tr>
                     </table>
                   </td></tr>
                     </table>

                     <table border="0" cellpadding="0" cellspacing="0" width="80">
                   <tr><td height=30><img width=1 height=10 src="dot.gif"></td></tr>
                     </table>
                     </font>
                     </td>
                    
                     </tr>
                   </table></td>
                                      <td width="100%">
                   <table border="0" cellpadding="0" cellspacing="0" width="100%">
                     <tr><td>
                       <table border="0" cellpadding="0" cellspacing="0" width="100%">
                     <tr>
                     <td width="100%" align="right">
                     </td>
                     </tr>
                   </table></td></tr>
                     <tr><td>
                       <table border="0" cellpadding="10" cellspacing="0" width="100%">
                     <tr><td width="650"></td>
                       <td width="0%"></td></tr>
                     <tr><td>
                       <font face="Geneva,Verdana,Helvetica,Arial">                       
                      <!-- Body -->
                <ul>
                    <li>The great Roberto launches RareWares. Hapiness ensues.
                        (actually that happened in 2001, but oh well...)</li>

                    <li>blahblah v2</li>

                    <li>Filler</li>

                    <li>Some more filler</li>

                    <li>Enough filler!</li>

                </ul>
               
                       </blockquote></font>
               
                       <br><br>
               
                       </font>
                     </td></tr>
                   </table>
                     </td></tr>
                   </table>
                   </td>
                   </tr>
                 </table></td>
                 </tr>
               </table> 
             <!-- End Timeline Module 1999 -->

         <!-- End Timeline -->

           </font>

        <br><br>

        </font>
      </td></tr>
    </table>
      </td></tr>
    </table>
    </td>
    </tr>
  </table></td>
  </tr>
</table> 
 
<?php include("footer.htm") ?>

</body>
</html>
